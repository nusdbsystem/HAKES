...
#include "index/impl/code_distance/code_distance.h"
...
void MultiIndexQuantizer::add(idx_t /*n*/, const float* /*x*/) {
    ...
    assert(!"This index has virtual elements, it does not support add");
}
...
void MultiIndexQuantizer::reset() {
    ...
    assert(!"This index has virtual elements, it does not support reset");
}
...
MultiIndexQuantizer2::MultiIndexQuantizer2(
    ...
    //         "Provided sub-index has incorrect size");
    assert(assign_index_0->d == pq.dsub && assign_index_1->d == pq.dsub);
