...
#include "index/ext/IndexFlatL.h"

#include <cstring>

#include "index/utils/Heap.h"
...
IndexFlatL::IndexFlatL(idx_t d, MetricType metric)
    : IndexFlatCodesL(sizeof(float) * d, d, metric) {}

void IndexFlatL::search(idx_t n, const float* x, idx_t k, float* distances,
  ...
  if (metric_type == METRIC_INNER_PRODUCT) {
    ...
    knn_inner_product(x, xb, d, n, ntotal, &res, sel);
  } else if (metric_type == METRIC_L2) {
    float_maxheap_array_t res = {size_t(n), size_t(k), labels, distances};
    knn_L2sqr(x, xb, d, n, ntotal, &res, nullptr, sel);
    // } else if (is_similarity_metric(metric_type)) {
    //   float_minheap_array_t res = {size_t(n), size_t(k), labels, distances};
    //   knn_extra_metrics(x, xb, d, n, ntotal, metric_type, metric_arg, &res);
    // } else {
...
void IndexFlatL::range_search(idx_t n, const float* x, float radius,
  ...
  switch (metric_type) {
    case METRIC_INNER_PRODUCT:
      range_search_inner_product(x, xb, d, n, ntotal, radius, result, sel);
      break;
    case METRIC_L2:
      range_search_L2sqr(x, xb, d, n, ntotal, radius, result, sel);
      break;
    default:
...
void IndexFlatL::compute_distance_subset(idx_t n, const float* x, idx_t k,
  ...
  switch (metric_type) {
    case METRIC_INNER_PRODUCT:
      fvec_inner_products_by_idx(distances, x, xb, labels, d, n, k);
      break;
    case METRIC_L2:
      fvec_L2sqr_by_idx(distances, x, xb, labels, d, n, k);
      break;
    default:
      // FAISS_THROW_MSG("metric type not supported");
      assert(!"metric type not supported");
...
struct FlatL2Dis : FlatCodesDistanceComputer {
  ...
  explicit FlatL2Dis(const IndexFlatL& storage, const float* q = nullptr)
      : FlatCodesDistanceComputer(storage.codes.data(), storage.code_size),
        d(storage.d),
        nb(storage.ntotal),
        q(q),
        // b(storage.get_xb()),
        b((const float*)storage.codes.data()),
        ndis(0),
        storage(storage) {}

  ~FlatL2Dis() { storage.release_xb(); }
...
struct FlatIPDis : FlatCodesDistanceComputer {
  ...
  explicit FlatIPDis(const IndexFlatL& storage, const float* q = nullptr)
      : FlatCodesDistanceComputer(storage.codes.data(), storage.code_size),
        d(storage.d),
        nb(storage.ntotal),
        q(q),
...
// the shared access to flat codes obtained, released when the returned distance
// computer is deleted
FlatCodesDistanceComputer* IndexFlatL::get_FlatCodesDistanceComputer() const {
  if (metric_type == METRIC_L2) {
    ...
    // no way to release the xb
    // FAISS_THROW_MSG("metric type not supported");
    assert(!"metric type not supported");
    // return get_extra_distance_computer(d, metric_type, metric_arg, ntotal,
    //                                    get_xb());
  }
}

void IndexFlatL::reconstruct(idx_t key, float* recons) const {
...
void IndexFlatL::sa_encode(idx_t n, const float* x, uint8_t* bytes) const {
  if (n > 0) {
    memcpy(bytes, x, sizeof(float) * d * n);
  }
}

void IndexFlatL::sa_decode(idx_t n, const uint8_t* bytes, float* x) const {
  if (n > 0) {
...
//   explicit FlatL2WithNormsDis(const IndexFlatLL2& storage,
...
//   ~FlatL2WithNormsDis() { storage.release_xb(); }
...
//   // compute four distances
//   void distances_batch_4(const idx_t idx0, const idx_t idx1, const idx_t
//   idx2,
//                          const idx_t idx3, float& dis0, float& dis1,
//                          float& dis2, float& dis3) final override {
//     ndis += 4;
...
// void IndexFlatLL2::sync_l2norms() {
//   // cached_l2norms.resize(ntotal);
//   // fvec_norms_L2sqr(cached_l2norms.data(),
//   //                  reinterpret_cast<const float*>(codes.data()), d,
//   ntotal);
// }

// void IndexFlatLL2::clear_l2norms() {
...
// the shared access to flat codes obtained, released when the returned distance
// computer is deleted
FlatCodesDistanceComputer* IndexFlatLL2::get_FlatCodesDistanceComputer() const {
  // if (metric_type == METRIC_L2) {
  ...
  return IndexFlatL::get_FlatCodesDistanceComputer();
}

/***************************************************
 * IndexFlatL1D (not used)
 ***************************************************/

// IndexFlatL1D::IndexFlatL1D(bool continuous_update)
//     : IndexFlatLL2(1), continuous_update(continuous_update) {}

// /// if not continuous_update, call this between the last add and
// /// the first search
// void IndexFlatL1D::update_permutation() {
//   perm.resize(ntotal);
//   if (ntotal < 1000000) {
...
// void IndexFlatL1D::add(idx_t n, const float* x) {
//   IndexFlatLL2::add(n, x);
//   if (continuous_update) update_permutation();
// }

// void IndexFlatL1D::reset() {
//   IndexFlatLL2::reset();
//   perm.clear();
// }

// void IndexFlatL1D::search(idx_t n, const float* x, idx_t k, float* distances,
//                           idx_t* labels, const SearchParameters* params)
//                           const {
//   FAISS_THROW_IF_NOT_MSG(!params, "search params not supported for this
//   index"); FAISS_THROW_IF_NOT(k > 0); FAISS_THROW_IF_NOT_MSG(perm.size() ==
//   ntotal,
