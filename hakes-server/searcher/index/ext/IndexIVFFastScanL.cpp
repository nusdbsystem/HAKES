...
#include "index/IndexIVFPQ.h"
#include "index/ext/BlockInvertedListsL.h"
#include "index/impl/AuxIndexStructures.h"
#include "index/impl/pq4_fast_scan.h"
#include "index/impl/simd_result_handlers.h"
#include "index/utils/distances.h"
#include "index/utils/quantize_lut.h"
...
IndexIVFFastScanL::IndexIVFFastScanL(Index* quantizer, size_t d, size_t nlist,
                                     size_t code_size, MetricType metric)
    : IndexIVFL(quantizer, d, nlist, code_size, metric) {
...
IndexIVFFastScanL::IndexIVFFastScanL(Index* quantizer, size_t d, size_t nlist,
                                     size_t code_size, bool balanced_assign,
                                     int balance_k, MetricType metric)
    : IndexIVFL(quantizer, d, nlist, code_size, metric) {
  // unlike other indexes, we prefer no residuals for performance reasons.
  by_residual = false;
  use_balanced_assign_ = balanced_assign;
  balance_k_ = balance_k;
  // FAISS_THROW_IF_NOT(metric == METRIC_L2 || metric == METRIC_INNER_PRODUCT);
  assert(metric == METRIC_L2 || metric == METRIC_INNER_PRODUCT);
}

void IndexIVFFastScanL::init_fastscan(size_t M, size_t nbits, size_t nlist,
                                      MetricType /* metric */, int bbs) {
  assert(bbs % 32 == 0);
  assert(nbits == 4);  // nbits now must be 4

  this->M = M;
  this->nbits = nbits;
  this->bbs = bbs;
  ksub = (1 << nbits);  // ksub is the number of centroids in a field
  M2 = roundup(M, 2);
  code_size = M2 / 2;
...
void IndexIVFFastScanL::init_code_packer() {
  auto bil = dynamic_cast<BlockInvertedListsL*>(invlists);
  assert(bil);
  bil->set_code_packer(get_CodePacker());
}

IndexIVFFastScanL::~IndexIVFFastScanL() = default;

/*********************************************************
 * Code management functions
 *********************************************************/

void IndexIVFFastScanL::add_with_ids(idx_t n, const float* x,
                                     const idx_t* xids) {
  add_with_ids(n, x, xids, false, nullptr);
}

void IndexIVFFastScanL::get_add_assign(idx_t n, const float* x, idx_t* assign) {
  if (assign == nullptr) {
    // printf("get_add_assign: assign is nullptr\n");
    return;
  }
  if (!use_balanced_assign_ || balance_k_ == 1) {
    quantizer->assign(n, x, assign);
  } else {
    std::unique_ptr<idx_t[]> cand_idx =
        std::unique_ptr<idx_t[]>(new idx_t[n * balance_k_]);
    quantizer->assign(n, x, cand_idx.get(), balance_k_);
    for (idx_t i = 0; i < n; i++) {
      idx_t selected = cand_idx[i * balance_k_];
      size_t selected_load = invlists->list_size(selected);
      for (idx_t j = 1; j < balance_k_; j++) {
        size_t cand_load = invlists->list_size(cand_idx[i * balance_k_ + j]);
        if (selected_load > cand_load) {
          selected = cand_idx[i * balance_k_ + j];
          selected_load = cand_load;
        }
      }
      assign[i] = selected;
    }
...
void IndexIVFFastScanL::add_with_ids(idx_t n, const float* x, const idx_t* xids,
                                     bool assigned, idx_t* assign) {
  ...
  if (n > bs) {
    ...
    for (idx_t i0 = 0; i0 < n; i0 += bs) {
      ...
      //   printf(
      //       "IndexIVFFastScanL::add_with_ids %zd/%zd, time %.2f/%.2f, RSS "
      //       "%zdMB\n",
      //       size_t(i1), size_t(n), elapsed_time, total_time, mem);
      // }
      //   add_with_ids(i1 - i0, x + i0 * d, xids ? xids + i0 : nullptr);
      add_with_ids(i1 - i0, x + i0 * d, xids + i0, assigned,
                   assigned ? assign + i0 : nullptr);
    }
    return;
  }
  InterruptCallback::check();

  // std::chrono::high_resolution_clock::time_point assign_start =
  //     std::chrono::high_resolution_clock::now();
  std::unique_ptr<idx_t[]> idx(new idx_t[n]);

  if (!assigned) {
    get_add_assign(n, x, idx.get());

    if (assign) {
      std::memcpy(assign, idx.get(), n * sizeof(idx_t));
    }
  } else {
    memcpy(idx.get(), assign, n * sizeof(idx_t));
  }

  // std::chrono::high_resolution_clock::time_point assign_end =
  //     std::chrono::high_resolution_clock::now();
  // std::chrono::duration<double> elapsed_seconds_assign =
  ...
  //     std::chrono::high_resolution_clock::now();
  // std::chrono::duration<double> elapsed_seconds_encode =
  //     encode_end - encode_start;

  BlockInvertedListsL* bil = static_cast<BlockInvertedListsL*>(invlists);
  ...
  std::stable_sort(order.begin(), order.end(),
                   [&idx](idx_t a, idx_t b) { return idx[a] < idx[b]; });
  ...
  while (i0 < n) {
    ...
    for (idx_t i = i0; i < i1; i++) {
      memcpy(list_codes.data() + (i - i0) * code_size,
             flat_codes.data() + order[i] * code_size, code_size);
    }
    {
      // std::chrono::high_resolution_clock::time_point begin =
      //     std::chrono::high_resolution_clock::now();
      // obtain the exclusive lock on the list for addition
      bil->lock_exclusive(list_no);
      size_t list_size = bil->list_size(list_no);
      bil->resize_nolock(list_no, list_size + i1 - i0);

      for (idx_t i = i0; i < i1; i++) {
        size_t ofs = list_size + i - i0;
        idx_t id = xids[order[i]];
        bil->lists_[list_no].ids_[ofs] = id;
      }

      pq4_pack_codes_range(list_codes.data(), M, list_size, list_size + i1 - i0,
                           bbs, M2, bil->lists_[list_no].codes_.data());
      bil->unlock_exclusive(list_no);
      // std::chrono::high_resolution_clock::time_point end =
      //     std::chrono::high_resolution_clock::now();
      // std::chrono::duration<double> elapsed_seconds = end - begin;

      // std::chrono::duration<double> after_assign = encode_start - assign_end;
      // std::chrono::duration<double> after_encode = begin - encode_end;
  ...
  // std::chrono::duration<float> total_seconds =
  //     std::chrono::high_resolution_clock::now() - start;
}

CodePacker* IndexIVFFastScanL::get_CodePacker() const {
...
// guoyu0606 related to hamming disabled
namespace {

// template <class C, typename dis_t>
// void estimators_from_tables_generic(const IndexIVFFastScanL& index,
//                                     const uint8_t* codes, size_t ncodes,
//                                     const dis_t* dis_table, const int64_t*
//                                     ids, float bias, size_t k, typename C::T*
//                                     heap_dis, int64_t* heap_ids, const
//                                     NormTableScaler* scaler) {
//   // C is the heap impl, T is the distance metric
...
void IndexIVFFastScanL::compute_LUT_uint8(size_t n, const float* x,
  ...
  for (int64_t i = 0; i < n; i++) {
    ...
    quantize_LUT_and_bias(nprobe, M, ksub, lut_is_3d, t_in, b_in, t_out, M2,
                          b_out, normalizers + 2 * i, normalizers + 2 * i + 1);
  }
}
...
void IndexIVFFastScanL::search(idx_t n, const float* x, idx_t k,
  ...
  assert(!params || paramsi);
  search_preassigned(n, x, k, nullptr, nullptr, distances, labels, false,
                     paramsi);
}

void IndexIVFFastScanL::search_preassigned(idx_t n, const float* x, idx_t k,
                                           const idx_t* assign,
...
void IndexIVFFastScanL::search_preassigned_new(
    ...
    const TagChecker<idx_t>* del_checker, IndexIVFStats* /*stats*/) const {
  size_t nprobe = this->nprobe;
  if (params) {
    // FAISS_THROW_IF_NOT_MSG(!params->quantizer_params,
    //                        "quantizer params not supported");
...
template <class C>
ResultHandlerCompare<C, true>* make_knn_handler_fixC(int impl, idx_t n, idx_t k,
                                                     float* distances,
                                                     idx_t* labels) {
...
SIMDResultHandlerToFloat* make_knn_handler(bool is_max, int impl, idx_t n,
                                           idx_t k, float* distances,
                                           idx_t* labels) {
  if (is_max) {
    return make_knn_handler_fixC<CMax<uint16_t, int64_t>>(impl, n, k, distances,
                                                          labels);
  } else {
    return make_knn_handler_fixC<CMin<uint16_t, int64_t>>(impl, n, k, distances,
                                                          labels);
  }
}

using CoarseQuantized = IndexIVFFastScanL::CoarseQuantized;
...
int compute_search_nslice(const IndexIVFFastScanL* index, size_t n,
                          size_t nprobe) {
  int nslice;
  if (n <= omp_get_max_threads()) {
    ...
    nslice =
        roundup(std::max(size_t(n / nq_ok), size_t(1)), omp_get_max_threads());
  } else {
...
void IndexIVFFastScanL::search_dispatch_implem(
...
void IndexIVFFastScanL::search_dispatch_implem_new(
  ...
  std::unique_ptr<RH> handler(
      make_knn_handler(is_max, impl, n, k, distances, labels));
  size_t ndis = 0, nlist_visited = 0;

  search_implem_12_new(n, x, k, *handler.get(), cq, &ndis, &nlist_visited,
                       scaler, del_checker);
...
void IndexIVFFastScanL::range_search_dispatch_implem(
    idx_t n, const float* x, float radius, RangeSearchResult& rres,
    const CoarseQuantized& cq_in, const NormTableScaler* scaler) const {
  bool is_max = !is_similarity_metric(metric_type);
  ...
  if (!multiple_threads) {  // single thread
    ...
    if (is_max) {
      handler.reset(
          new RangeHandler<CMax<uint16_t, int64_t>, true>(rres, radius, 0));
    } else {
      handler.reset(
          new RangeHandler<CMin<uint16_t, int64_t>, true>(rres, radius, 0));
    }
    if (impl == 12) {
      search_implem_12(n, x, *handler.get(), cq, &ndis, &nlist_visited, scaler);
    } else if (impl == 10) {
      search_implem_10(n, x, *handler.get(), cq, &ndis, &nlist_visited, scaler);
    } else {
      // FAISS_THROW_FMT("Range search implem %d not impemented", impl);
      assert(!"Range search implem not implemented");
    }
  } else {
    // explicitly slice over threads
    int nslice = compute_search_nslice(this, n, cq.nprobe);
...
#pragma omp parallel for reduction(+ : ndis, nlist_visited)
      for (int slice = 0; slice < nslice; slice++) {
        ...
        std::unique_ptr<SIMDResultHandlerToFloat> handler;
        if (is_max) {
          handler.reset(new PartialRangeHandler<CMax<uint16_t, int64_t>, true>(
              pres, radius, 0, i0, i1));
        } else {
          handler.reset(new PartialRangeHandler<CMin<uint16_t, int64_t>, true>(
              pres, radius, 0, i0, i1));
        }

        if (impl == 12 || impl == 13) {
          search_implem_12(i1 - i0, x + i0 * d, *handler.get(), cq_i, &ndis,
                           &nlist_visited, scaler);
        } else {
          search_implem_10(i1 - i0, x + i0 * d, *handler.get(), cq_i, &ndis,
                           &nlist_visited, scaler);
        }
      }
      pres.finalize();
...
// template <class C>
// void IndexIVFFastScanL::search_implem_1(idx_t n, const float* x, idx_t k,
//                                         float* distances, idx_t* labels,
//                                         const CoarseQuantized& cq,
//                                         const NormTableScaler* scaler) const
//                                         {
//   assert(orig_invlists);
...
//       estimators_from_tables_generic<C>(*this, codes.get(), ls, LUT,
//       ids.get(),
//                                         bias, k, heap_dis, heap_ids, scaler);
//       nlist_visited++;
//       ndis++;
//     }
...
// template <class C>
// void IndexIVFFastScanL::search_implem_2(idx_t n, const float* x, idx_t k,
//                                         float* distances, idx_t* labels,
//                                         const CoarseQuantized& cq,
//                                         const NormTableScaler* scaler) const
//                                         {
...
void IndexIVFFastScanL::search_implem_10(idx_t n, const float* x,
                                         SIMDResultHandlerToFloat& handler,
                                         const CoarseQuantized& cq,
                                         size_t* ndis_out, size_t* nlist_out,
                                         const NormTableScaler* scaler) const {
  size_t dim12 = ksub * M2;
  AlignedTable<uint8_t> dis_tables;
  AlignedTable<uint16_t> biases;
  ...
  for (idx_t i = 0; i < n; i++) {
    ...
    for (idx_t j = 0; j < nprobe; j++) {
      ...
      pq4_accumulate_loop(1, roundup(ls, bbs), bbs, M2, codes.get(), LUT,
                          handler, scaler);
...
void IndexIVFFastScanL::search_implem_12(idx_t n, const float* x,
                                         SIMDResultHandlerToFloat& handler,
                                         const CoarseQuantized& cq,
                                         size_t* ndis_out, size_t* nlist_out,
                                         const NormTableScaler* scaler) const {
...
void IndexIVFFastScanL::search_implem_14(idx_t n, const float* x, idx_t k,
                                         float* distances, idx_t* labels,
                                         const CoarseQuantized& cq, int impl,
                                         const NormTableScaler* scaler) const {
  if (n == 0) {  // does not work well with reservoir
    return;
  }
...
#pragma omp for
    for (idx_t cluster = 0; cluster < ses.size(); cluster++) {
      ...
      pq4_accumulate_loop_qbs(qbs, list_size, M2, codes.get(), LUT.get(),
                              *handler.get(), scaler);
    }
...
#pragma omp critical
    {
      ...
      for (std::set<int>::iterator it = q_set.begin(); it != q_set.end();
           ++it) {
        add_local_results(local_dis.data() + *it * k,
                          local_idx.data() + *it * k, distances + *it * k,
                          labels + *it * k);
      }
    }
#pragma omp barrier
...
void IndexIVFFastScanL::reconstruct_from_offset(int64_t list_no, int64_t offset,
                                                float* recons) const {
  // // unpack codes
  ...
  //   uint8_t c = pq4_get_packed_element(list_codes.get(), bbs, M2, offset, m);
  //   bsw.write(c, nbits);
  // }
  ...
  //   }
  // }
  assert(!"reconstruct from offset not implemented");
}

void IndexIVFFastScanL::reconstruct_orig_invlists() {
  // assert(orig_invlists != nullptr);
  // assert(orig_invlists->list_size(0) == 0);
  ...
  //     BitstringWriter bsw(code.data(), code_size);
  //     for (size_t m = 0; m < M; m++) {
  //       uint8_t c = pq4_get_packed_element(codes.get(), bbs, M2, offset, m);
  //       bsw.write(c, nbits);
  //     }
  ...
  //   }
  // }
  assert(!"reconstruct orig invlists not implemented");
}

// IVFFastScanStatsL IVFFastScan_statsL;
