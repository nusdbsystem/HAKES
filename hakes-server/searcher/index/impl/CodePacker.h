...
#ifndef HAKES_SEARCHWORKER_INDEX_IMPL_CODEPACKER_H_
#define HAKES_SEARCHWORKER_INDEX_IMPL_CODEPACKER_H_

#include "index/MetricType.h"
...
struct CodePacker {
  ...
  virtual void pack_1(
      const uint8_t* flat_code,  // code to write to the block, size code_size
...
struct CodePackerFlat : CodePacker {
  ...
  void pack_1(const uint8_t* flat_code, size_t offset,
              uint8_t* block) const final;
  void unpack_1(const uint8_t* block, size_t offset,
                uint8_t* flat_code) const final;
...
#endif  // HAKES_SEARCHWORKER_INDEX_IMPL_CODEPACKER_H_
