...
#ifndef HAKES_SEARCHWORKER_INDEX_IMPL_LOOKUP_TABLE_SCALER_H
#define HAKES_SEARCHWORKER_INDEX_IMPL_LOOKUP_TABLE_SCALER_H
...
struct DummyScaler {
    ...
    inline simd32uint8 lookup(const simd32uint8&, const simd32uint8&) const {
        ...
        assert("DummyScaler::lookup should not be called.");
    ...
    inline simd16uint16 scale_lo(const simd32uint8&) const {
        ...
        assert("DummyScaler::scale_lo should not be called.");
    ...
    inline simd16uint16 scale_hi(const simd32uint8&) const {
        ...
        assert("DummyScaler::scale_hi should not be called.");
        return simd16uint16(0);
    }
    ...
    inline dist_t scale_one(const dist_t&) const {
        ...
        assert("DummyScaler::scale_one should not be called.");
        return 0;
    }
};
...
#endif  // HAKES_SEARCHWORKER_INDEX_IMPL_LOOKUP_TABLE_SCALER_H
