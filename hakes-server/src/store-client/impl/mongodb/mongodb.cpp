...
MongoDB::MongoDB(const std::string& addr, const std::string& db_name,
                 const std::string& collection_name)
    : Store(ValidateMongoDBAddress(addr)), counter_key_(collection_name) {
  try {
    ...
    auto counter_doc =
        counters_.find_one(document{} << "_id" << counter_key_ << finalize);

    if (!counter_doc) {
      counters_.insert_one(document{} << "_id" << counter_key_ << "seq"
                                      << int64_t(0) << finalize);
    }
  } catch (const std::exception& e) {
    throw std::runtime_error(std::string("Failed to connect to MongoDB: ") +
                             e.what());
  }
}

MongoDB::~MongoDB() { Disconnect(); }
...
void MongoDB::Disconnect() { client_.reset(); }
...
std::vector<std::vector<uint8_t>> MongoDB::GetNextXids(int batch_size) {
  try {
    ...
    throw std::runtime_error(std::string("Failed to get next xids: ") +
                             e.what());
  }
}
  ...
  try {
    ...
    for (const auto& xid : xids) {
      in_array << bsoncxx::types::b_binary{
          bsoncxx::binary_sub_type::k_uuid, static_cast<uint32_t>(xid.size()),
...
bool MongoDB::Delete(const std::string& key) {
  try {
    auto result =
        collection_.delete_one(document{} << "key" << key << finalize);
...
bool MongoDB::GetByUsername(const std::string& username, User* user) {
  if (user == nullptr) {
    return false;
  }
  ...
  auto result = users_.find_one(filter.view());
  if (!result) {
    return false;
  }
...
bool MongoDB::CreateUser(const User& user) {
  ...
  auto result = users_.insert_one(doc.view());
  return static_cast<bool>(result);
}

bool MongoDB::UpdateUser(const User& user) {
  auto filter = document{} << "username" << user.Username() << finalize;
  ...
  auto set_doc = document{} << "password_hash" << user.PasswordHash() << "roles"
                            << roles_array << finalize;

  auto update_doc = document{} << "$set" << set_doc.view() << finalize;

  auto result = users_.update_one(filter.view(), update_doc.view());
  return result && result->matched_count() > 0;
}

bool MongoDB::DeleteUser(const std::string& username) {
  auto filter = document{} << "username" << username << finalize;

  auto result = users_.delete_one(filter.view());
  return result && result->deleted_count() > 0;
}

User MongoDB::BsonToUser(const bsoncxx::document::view& doc) {
  std::string username;
  std::string password_hash;
  std::vector<UserRole> roles;

  auto username_elem = doc["username"];
  if (username_elem && username_elem.type() == bsoncxx::type::k_string) {
    username = std::string(username_elem.get_string().value);
  }

  auto password_elem = doc["password_hash"];
  if (password_elem && password_elem.type() == bsoncxx::type::k_string) {
    password_hash = std::string(password_elem.get_string().value);
  }

  auto roles_elem = doc["roles"];
  if (roles_elem && roles_elem.type() == bsoncxx::type::k_array) {
    for (const auto& elem : roles_elem.get_array().value) {
      if (elem.type() == bsoncxx::type::k_string) {
        roles.push_back(
            UserRoleFromString(std::string(elem.get_string().value)));
      }
    }
  }

  return User(username, password_hash, roles);
}

bsoncxx::document::value MongoDB::UserToBson(const User& user) {
  array roles_array{};
  for (const auto& role : user.Roles()) {
    roles_array << ToString(role);
  }
