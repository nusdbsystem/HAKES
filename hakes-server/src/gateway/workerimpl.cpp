...
bool WorkerImpl::HandleLogin(uint64_t handle_id, const std::string& input,
  ...
  try {
    ...
    std::cout << "Login attempt for user: " << username << "\n";

    // Get user from store
    hakes_store::User user;
    if (!store_->GetByUsername(username, &user)) {
    ...
    for (size_t i = 0; i < roles.size(); ++i) {
      if (i > 0)
        roles_str += ",";
      roles_str += hakes_store::ToString(roles[i]);
    }
    if (roles_str.empty()) {
      roles_str = "user";
    }
    ...
    std::string token = create_jwt_token(username, roles_str, exp);
