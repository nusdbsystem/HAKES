...
#include "index/impl/CodePacker.h"
...
#pragma omp single
            ...
            assert(!"parallel_mode not supported");
        }
    } // parallel section
...
void IndexIVF::update_vectors(int n, const idx_t* new_ids, const float* x) {
    ...
    assert(direct_map.type == DirectMap::Array);
...
void IndexIVF::check_compatible_for_merge(const Index& otherIndex) const {
    ...
    assert(other);
    assert(other->d == d);
    assert(other->nlist == nlist);
    assert(quantizer->ntotal == other->quantizer->ntotal);
    assert(other->code_size == code_size);
    assert(typeid(*this) == typeid(*other));
    assert(this->direct_map.no() && other->direct_map.no());
