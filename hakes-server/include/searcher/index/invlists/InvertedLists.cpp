...
InvertedListsIterator* InvertedLists::get_iterator(size_t /*list_no*/) const {
    ...
    assert(!"get_iterator is not supported");
}
...
size_t InvertedLists::copy_subset_to(
    ...
    //         subset_type);
    assert(nlist == oivf.nlist);
    assert(code_size == oivf.code_size);
    assert(subset_type >= 0 && subset_type <= 4);
...
// guoyu0605 disabled all unused invlists impl
