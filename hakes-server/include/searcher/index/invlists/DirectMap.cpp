...
#include "index/invlists/DirectMap.h"
...
#include "index/impl/IDSelector.h"
...
void DirectMap::set_type(
    ...
    assert(new_type == NoMap || new_type == Array || new_type == Hashtable);
    ...
    for (size_t key = 0; key < invlists->nlist; key++) {
        ...
        if (new_type == Array) {
            for (long ofs = 0; ofs < list_size; ofs++) {
                ...
                assert(0 <= idlist[ofs] && idlist[ofs] < ntotal);
...
idx_t DirectMap::get(idx_t key) const {
    if (type == Array) {
        ...
        assert(res != hashtable.end());
        return res->second;
    } else {
        // FAISS_THROW_MSG("direct map not initialized");
        assert(!"direct map not initialized");
    }
}
...
void DirectMap::check_can_add(const idx_t* ids) {
    if (type == Array && ids) {
        ...
        assert(!"cannot have array direct map and add with ids");
    }
}
...
#pragma omp parallel for
        ...
        assert(!"remove not supported with this direct_map format");
    }
    return nremove;
}
