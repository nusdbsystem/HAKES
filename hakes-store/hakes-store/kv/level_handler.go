...
func (s *levelHandler) deleteTables(toDel []*table.Table) error {
	...
	toDelMap := make(map[string]struct{})
	for _, t := range toDel {
...
func (s *levelHandler) close() error {
	...
	var err error
	for _, t := range s.tables {
		if closeErr := t.CloseTable(); closeErr != nil && err == nil {
			err = closeErr
		}
	}
...
func (s *levelHandler) get(key []byte) (y.ValueStruct, error) {
	...
	for _, th := range tables {
		// ValueCopy is not part of the iterator interface
...
func (s *levelHandler) appendIterators(iters []y.Iterator, opt *IteratorOptions) []y.Iterator {
	...
	return append(iters, table.NewConcatIterator(tables, topt, nil))
}
