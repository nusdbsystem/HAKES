...
func (itr *blockIterator) setIdx(i int) {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			fmt.Fprintf(&debugBuf, "Table ID: %v\nBlock ID: %d\nEntry Idx: %d\nData len: %d\n"+
				"StartOffset: %d\nEndOffset: %d\nEntryOffsets len: %d\nEntryOffsets: %v\n",
...
func (t *Table) NewIteratorWithPrefetchBuffer(opt int, prefetchOpts PrefetchOptions) y.Iterator {
	t.IncrRef() // Important.
	ti := &Iterator{
		t:   t,
		opt: opt,
		pb:  BuildPrefetchBuffer(prefetchOpts, t.CSSF, int(t.Size())),
	}
	return ti
}
...
func (itr *Iterator) moveToBPos() {
	block, err := itr.t.block(itr.bpos, itr.useCache(), itr.pb)
	if err != nil {
		itr.err = err
		return
	}
...
type ConcatIterator struct {
	...
	prefetchOpts PrefetchOptions
}

// NewConcatIterator creates a new concatenated iterator
// prefetch is an optional argument when set NewIterator will be created with it.
func NewConcatIterator(tbls []*Table, opt int, prefetchOpts *PrefetchOptions) *ConcatIterator {
	...
	if prefetchOpts != nil {
		ret.prefetchOpts = *prefetchOpts // hold a copy
	}
	return ret
}
...
func (s *ConcatIterator) setIdx(idx int) {
	...
	if s.iters[idx] == nil {
		if s.prefetchOpts.Type != NoPrefetch {
			s.iters[idx] = s.tables[idx].NewIteratorWithPrefetchBuffer(s.options, s.prefetchOpts)
		} else {
			// old logic
