...
int main(int argc, char* argv[]) {
  ...
  if (argc != 6) {
    fprintf(stderr, "Usage: port data_path enclave_file cluster_size server_id\n");
    exit(1);
  }
  // parse the port
  ...
  std::string enclave_file = argv[3];
  int cluster_size = std::stoi(argv[4]);
  int server_id = std::stoi(argv[5]);
