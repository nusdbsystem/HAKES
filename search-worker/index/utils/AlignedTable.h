...
#ifndef HAKES_SEARCHWORKER_INDEX_UTILS_ALIGNEDTABLE_H_
#define HAKES_SEARCHWORKER_INDEX_UTILS_ALIGNEDTABLE_H_
...
struct AlignedTableTightAlloc {
    ...
    void resize(size_t n) {
        ...
        if (n > 0) {
            ...
            int ret = platform_memalign((void**)&new_ptr, A, n * sizeof(T));
            // new_ptr = (T*)malloc(n * sizeof(T));
            if (ret != 0) {
            // if (new_ptr == nullptr) {
                throw std::bad_alloc();
            }
...
#endif  // HAKES_SEARCHWORKER_INDEX_UTILS_ALIGNEDTABLE_H_
