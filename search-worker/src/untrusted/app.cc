...
std::unique_ptr<float[]> decode_hex_floats(const std::string& vecs_str,
  ...
  auto decrypted_vecs = hakes::decrypt_content_with_key_aes(
      (const uint8_t*)vecs_bytes.data(), vecs_bytes.size(),
  ...
  *count = decrypted_size / sizeof(float);
  ...
  std::memcpy(vecs.get(), decrypted_vecs.data(), decrypted_size);
  return vecs;
}
...
std::string encode_hex_floats(const float* vecs, size_t count) {
  ...
  auto encrypted_vecs = hakes::encrypt_content_with_key_aes(
      (const uint8_t*)vecs, count * sizeof(float),
      std::string((const char*)secret_key_buf, sizeof(secret_key_buf)));
  std::string ret = hakes::hex_encode(encrypted_vecs.data(), encrypted_size);
  return ret;
}
...
int main(int argc, char* argv[]) {
  ...
  search_worker::WorkerU worker(ENCLAVE_FILENAME);
  ...
  for (int i = 0; i < 10; i++) {
    ...
    add_req.vecs = encode_hex_floats(data + i * cfg.data_dim, cfg.data_dim);
    int64_t ids[1] = {i};
  ...
  search_req.vecs = encode_hex_floats(query, cfg.data_dim);
  ...
  printf("encoded_search_req: %s\n", encoded_search_req.c_str());
  ...
  printf("encoded_rerank_req: %s\n", encoded_rerank_req.c_str());
  ...
  if (!status) {
    ...
    printf("ids size: %ld\n", content_len);
    auto scores = decode_hex_floats(rerank_resp.scores, &content_len);
