...
int main(int argc, char* argv[]) {
  ...
  search_worker::WorkerU worker0(ENCLAVE_FILENAME);
  search_worker::WorkerU worker1(ENCLAVE_FILENAME);
  search_worker::WorkerU worker2(ENCLAVE_FILENAME);
  ...
  auto status = worker0.Search(encoded_search_req.c_str(), encoded_search_req.size(),
                          resp.get(), 4096 * 4096);
