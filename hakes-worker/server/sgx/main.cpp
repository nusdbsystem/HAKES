...
int main(int argc, char* argv[]) {
  ...
  if (argc != 5) {
    fprintf(stderr, "Usage: port config_file is_ow_action enclave_file\n");
    exit(1);
  }
  // parse the port
  ...
  auto data_manager = std::unique_ptr<hakes_worker::DataManagerU>(
      new hakes_worker::DataManagerU(enclave_file));
...
// UV_THREADPOOL_SIZE=10 SGX_AESM_ADDR=1 ./install/bin/hakes_server 2355 ../data/hakesworker/sample-config.json 0 ./install/lib/Worker_Enclave.signed.so
