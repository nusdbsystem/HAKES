...
src/untrusted/Enclave_u.c: $(SGX_EDGER8R) ${PROJECT_ROOT_DIR}/include/hakes-worker/trusted/Enclave.edl
	@echo Entering ./untrusted
	cd ./src/untrusted && $(SGX_EDGER8R) --untrusted ${PROJECT_ROOT_DIR}/include/hakes-worker/trusted/Enclave.edl --search-path ${PROJECT_ROOT_DIR}/include/ --search-path $(SGX_SDK)/include --search-path $(SGXSSL_DIR)/include --search-path $(SGX_RA_TLS_DIR)/include --search-path ${HAKES_ROOT} $(Enclave_Search_Dirs)
	@echo "GEN  =>  $@"
...
src/untrusted/crypto_ext.o: ${HAKES_ROOT}/utils/crypto_ext.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"
...
src/untrusted/http.o: ${HAKES_ROOT}/utils/http.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"

src/untrusted/client_req.o: ${HAKES_ROOT}/message/client_req.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"
...
src/untrusted/searchservice.o: ${HAKES_ROOT}/message/searchservice.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"

src/untrusted/kvservice.o: ${HAKES_ROOT}/message/kvservice.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"

## build files needed from other directory

src/untrusted/config.o: src/common/config.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"

src/untrusted/workerimpl.o: src/common/workerimpl.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"
...
App_Cpp_Objects := src/untrusted/base64.o src/untrusted/fileutil.o src/untrusted/hexutil.o src/untrusted/json.o src/untrusted/http.o src/untrusted/client_req.o src/untrusted/embed.o src/untrusted/searchservice.o src/untrusted/kvservice.o src/untrusted/data_manager_u.o src/untrusted/config.o src/untrusted/workerimpl.o src/untrusted/ocalls.o src/untrusted/channel_client_ocalls.o

# endpoint object files
App_Cpp_Objects += src/no-sgx/endpoint.o \
	src/no-sgx/openai_endpoint.o \
	src/no-sgx/huggingface_endpoint.o

libuntrusted_hakes_worker.a: src/untrusted/Enclave_u.o $(App_Cpp_Objects)
	ar -rcs $@ $^
	@echo "LINK => $@"

## Build worker app ##
app: src/untrusted/app.o src/untrusted/crypto_ext.o libuntrusted_hakes_worker.a
	$(CXX) $< src/untrusted/crypto_ext.o -o $@ -L. -l:libuntrusted_hakes_worker.a $(App_Link_Flags)
	@echo "LINK =>  $@"
...
server/sgx/hakes_worker.o: server/hakes_worker.cpp
	$(CXX) $(App_Cpp_Flags) $(Server_Additional_Include_Flags) -c $< -o $@
	@echo "CXX <= $<"
...
Server_Objects := server/sgx/service.o server/sgx/server.o server/sgx/hakes_worker.o server/sgx/main.o

hakes_server: $(Server_Objects) libuntrusted_hakes_worker.a
	$(CXX) $(Server_Objects) -o $@ -L. -l:libuntrusted_hakes_worker.a $(App_Link_Flags) $(Server_Additional_Link_Flags)
## Sources ###

### Clean command ###
clean:
	rm -f app src/untrusted/*.o $(App_Cpp_Objects) src/untrusted/Enclave_u.* libuntrusted_hakes_worker.a server/sgx/*.o hakes_server
