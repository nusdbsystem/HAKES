...
#include "hakes-worker/common/workerimpl.h"
...
bool WorkerImpl::HandleKvOp(uint64_t handle_id,
  ...
  if (request.data_type == HakesRequestDataType::kRaw) {
    ...
    if (config_.GetEmbedEndpointType() == "hakes-embed") {
      ...
      embed_request.encrypted_sample_ = request.data;
      embed_request.user_id_ = request.user_id;
      embed_request.key_service_address_ = request.key_service_address;
      embed_request.key_service_port_ = request.key_service_port;
      ...
      response.aux.push_back(embed_response.aux);
    } else {
  ...
  for (const auto& resp : add_resps) {
    ...
    response.aux.push_back(add_response.aux);
  }
...
bool WorkerImpl::HandleSearchOp(uint64_t handle_id,
  ...
  response.status = false;
  response.aux.reserve(2 + config_.ServerCount());
  ...
  search_request.user_id = request.user_id;
  search_request.ks_addr = request.key_service_address;
  search_request.ks_port = request.key_service_port;
  ...
  response.aux.push_back(search_response.aux);
  ...
  rerank_request.ks_addr = request.key_service_address;
  rerank_request.ks_port = request.key_service_port;
  ...
  for (const auto& rerank_resp : rerank_resps) {
    ...
    response.aux.push_back(rerank_response.aux);
  }
...
#else
  auto merged_result = dm_->MergeSearchResults(
      partial_results, request.k, request.user_id, request.key_service_address,
      request.key_service_port);
#endif
  ...
  response.aux.push_back(merged_result.aux);
