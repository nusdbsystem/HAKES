...
hakes::SearchWorkerRerankResponse DataManagerImpl::MergeSearchResults(
    const std::vector<hakes::SearchWorkerRerankResponse> resps, int k, const std::string /*user_id*/, const std::string /*ks_addr*/, const uint16_t /*ks_port*/) {
  ...
  for (int i = 0; i < resps.size(); i++) {
    ...
    merged_resp.aux.append(resps[i].aux);
  }
